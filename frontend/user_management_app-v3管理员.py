...
def login_page():
    ...
    if st.button("登录"):
        ...
        if response["success"]:
            ...
            # --- 获取用户信息 ---
            user_info_response = api_request('POST', '/auth/me', data={"token": st.session_state.access_token})
            st.write("调试: /auth/me 响应:", user_info_response) # <--- 添加调试信息
            if user_info_response["success"]:
                st.session_state.user_info = user_info_response["data"]["user"] # <--- 获取 user 对象

                # --- 修正：从嵌套的 user 对象中获取 is_superadmin ---
                st.session_state.is_admin = user_info_response["data"].get("user", {}).get("is_superadmin", False)
                # --- 修正结束 ---

                st.write(f"调试: 检测到的 is_admin: {st.session_state.is_admin}") # <--- 添加调试信息
                st.rerun()
            else:
                st.error(f"登录成功，但获取用户信息失败: {user_info_response['detail']}") # 登录成功后获取用户信息失败
                # 清理掉无效的 token，避免用户卡在登录状态
                st.session_state.access_token = None
                st.session_state.refresh_token = None
                st.session_state.user_info = None
                st.session_state.is_admin = False
...
def user_profile_page():
    ...
    if user_info and isinstance(user_info, dict):
        ...
        st.write(f"是否管理员: {'是' if user_info.get('is_superadmin', False) else '否'}") # 直接从 user_info 获取
        st.write(f"上次登录时间: {user_info.get('last_login', 'N/A')}")
    else:
        st.error("用户信息未正确加载，请重新登录")
        st.session_state.access_token = None # 清理状态
        st.session_state.refresh_token = None
        st.session_state.user_info = None
        st.session_state.is_admin = False
        st.rerun()
